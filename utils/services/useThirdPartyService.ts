...
import useThirdPartyAPI from '../apis/useThirdPartyAPI'
import useErrorHandler from '../useErrorHandler'

function useThirdPartyService() {
  const { apiErrHandler } = useErrorHandler()
  const { setList, setViewData, setViewId } = useDataContext<ThirdParty>()
  const { setTotalCount, page, perpage } = usePaginateContext()
  const { setSearch } = useSearchContext<ThirdPartyListRequest>()
  const [, setEditVisible] = usePopupContext('editForm')
  const [, setCreateVisible] = usePopupContext('createForm')
  const API = useThirdPartyAPI()
  const toast = useToast()

  const fetchList = async (req?: ThirdPartyListRequest) => {
    try {
  ...
  const doCreate = async (req: ThirdPartyCreateRequest) => {
    try {
  ...
  const doEdit = async (req: ThirdPartyEditRequest) => {
    try {
      await API.edit(req)
      setSearch((s) => ({ ...s }))
